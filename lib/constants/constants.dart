import "package:flutter/material.dart";

//colro used
const primaryColor = Color.fromRGBO(17, 159, 250, 1);
const secondaryColor = Colors.white;
const bgColor = Color.fromRGBO(247, 251, 254, 1);
const textColor = Colors.black;
const lightTExtColor = Colors.black26;
const trasparent = Colors.transparent;

const grey = Color.fromRGBO(148, 170, 220, 1);
const purple = Color.fromRGBO(165, 88, 179, 1);
const orange = Color.fromRGBO(251, 137, 13, 1);
const green = Color.fromRGBO(51, 173, 127, 1);
const red = Colors.red;

//default app padding
const appPadding = 16.0;

const String url = 'http://192.168.101.253:3000';
const String ip = '192.168.2.227';
const NoticeImage = "${url}/notice_upload";
const TeacherImage = "${url}/uploads/teacher_upload";
const ParentImage = "${url}uploads/parent_upload";
const userTeacherImg = "$url/uploads/teacher_upload";
const userParentImg = "$url/uploads/parent_upload";
